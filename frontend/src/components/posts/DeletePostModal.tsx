...
const DeletePostModal = () => {
    const showModal = useShowModal("DeletePost");
    ...
    const handleDeletePost = async( e: React.SubmitEvent<HTMLFormElement> ) => {
        e.preventDefault();
        try {
            const message = await deletePost( { groupId: +groupId!, postId: +postId! } );
                        ...
                        >
                            <Dialog.Panel className="w-5/6 max-w-5xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all p-10">
                                ...
                                <div className="text-center mt-5 text-2xl">
                                    <p>&iquest;Estas seguro que quieres eliminar esta publicacion?</p>
                                </div>
